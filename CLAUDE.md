...
**Privacy and safety (keep PRIVACY.md and SECURITY.md true).** teveus sends nothing on its own except the daily update check (`update.go`: one GET to api.github.com, off with `noUpdateCheck`, never for source builds, and it asks before installing): no telemetry or analytics. Any change that makes a new network request, stores new data, or changes a safeguard must update PRIVACY.md / SECURITY.md in the same change. File tools go through `Engine.sensitive` (credential stores, blocked in every mode) and `Engine.inProject` (outside the project always asks, except autopilot); new file tools must be added to `toolPaths`. Files in the config dir are written with `writePrivate` (0600).
