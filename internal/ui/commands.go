...
func init() {
	localCmds = []localCmd{
		...
		{"worktree", "Start a session in its own git worktree", "", func(m *Model, _ string) tea.Cmd { return m.newWorktree() }},
		{"resume", "Resume an earlier conversation in this folder", "", func(m *Model, _ string) tea.Cmd { m.openResumePicker(); return nil }},
		{"search", "Find text in this conversation and earlier ones", "", func(m *Model, arg string) tea.Cmd { return m.search(arg) }},
		...
		{"hooks", "Commands that run before or after tool calls", "", func(m *Model, _ string) tea.Cmd { return m.showHooks() }},
		...
		{"subagent", "Model for research subagents (Direct API): a cheaper one saves tokens", "", func(m *Model, arg string) tea.Cmd { return m.openSubagentPicker(arg) }},
